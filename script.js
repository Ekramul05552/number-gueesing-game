...
function startNewPlayerTurn() {
    gameHistory.push({
        player: currentPlayerIndex, // Stores the index of the current player (e.g., 0, 1)
        guesses: [], // Initializes an empty array to hold the player's guesses
        won: false // Sets the initial win status to false
    });
...
document.querySelector(".start-setup-btn").addEventListener("click", () => {
    ...
    if (totalGuessers < 1 || totalGuessers > 5) return;
